...
from datetime import datetime, date   # Импорт классов datetime и date из модуля datetime для работы с датами и временем
...
CURRENT_DATA: date = datetime.now().date() # Получение текущей даты
...
def get_dict_of_available_three_days_of_holiday(holidays: LIST_HOLIDAY) -> DICT_HOLIDAY:
    ...
    for holiday in holidays:
        ...
        holiday_date: date = datetime.strptime(holiday['date'], '%Y-%m-%d').date()
        ...
        if days_until >= 0:
            ...
            result[holiday['name']] = f"Страна: {holiday['countryCode']} | через {days_until} дней ({holiday['date']})"
            count += 1
